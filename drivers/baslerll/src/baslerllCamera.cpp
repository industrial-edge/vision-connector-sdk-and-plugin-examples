...
namespace VCA::baslerll
{
            ...
            if (StreamGrabber->GetWaitObject().Wait(s_imageAcquisitionTimeout))
            {
                ...
                if (ptrGrabResult.Succeeded())
                {
                    const VCA::SDK::v1::ImageDetail imageDetail(
                        ...
                        VCA::SDK::v1::ImageInformation(increaseImageSequenceCounter(), ptrGrabResult.GetSizeX(),
                                                       ptrGrabResult.GetSizeY(),
                                                       pixelTypeToString(ptrGrabResult.GetPixelType())),
                        VCA::SDK::v1::PTPInformation("Disabled", "0"));
                    auto imageData = std::make_unique<SDK::v1::ImageData>((unsigned char*)ptrGrabResult.Buffer(),
    ...
    void baslerllCamera::startImageAcquisition()
    {
        ...
        StreamGrabber->Open();
        bufferSize = static_cast<int>(camera->PayloadSize());
        numBuffers = 10;
    ...
    SDK::v1::CameraParameters baslerllCamera::getConfig()
    {
        ...
        auto widthParam = SDK::v1::CameraParameter::CreateParameter(
            SDK::v1::ParameterType::Int, "Width", to_string(camera->Width.GetValue()), "Image width", false);
        widthParam.setMin("1");
    ...
    SDK::v1::CameraParameterStatuses baslerllCamera::setConfig(const SDK::v1::CameraParameters& parametersToChange)
    {
        ...
        for (const auto& parameterToChange : parametersToChange)
        {
            ...
            {
                camera->TriggerMode.SetValue(parameterValue == "On" ? TriggerModeEnums::TriggerMode_On
                                                                    : TriggerModeEnums::TriggerMode_Off);
                status = true;
            }
