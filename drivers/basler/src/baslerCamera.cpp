...
namespace VCA::basler
{
    ...
    std::string baslerCamera::pixelTypeToString(EPixelType pixelType)
    {
        ...
        case PixelType_BGR10V1packed:
            return "BGR10V1packed";
        case PixelType_BGR10V2packed:
            return "BGR10V2packed";
    ...
    std::shared_ptr<VCA::SDK::v1::Image> baslerCamera::acquireImage()
    {
        ...
        if (ptrGrabResult->GrabSucceeded())
        {
            ...
            const VCA::SDK::v1::ImageDetail imageDetail(
                ...
                VCA::SDK::v1::ImageInformation(imageSequenceCounter(), ptrGrabResult->GetHeight(),
                                               ptrGrabResult->GetWidth(),
                                               pixelTypeToString(ptrGrabResult->GetPixelType())),
                VCA::SDK::v1::PTPInformation("Disabled", "0"));
            auto imageData = std::make_unique<SDK::v1::ImageData>((unsigned char*)ptrGrabResult->GetBuffer(),
    ...
    SDK::v1::CameraParameters baslerCamera::getConfig()
    {
        ...
        auto symbPrepare = [](const StringList_t& sl)
        {
            SDK::v1::Symbolics s;
            for (const auto& symb : sl)
            {
                s.push_back(symb.c_str());
            }
            return s;
        };

        CIntegerParameter width(m_camera->GetNodeMap(), "Width");
        auto widthParam = SDK::v1::CameraParameter::CreateParameter(SDK::v1::ParameterType::Int, "Width",
                                                                    to_string(width.GetValue()), "Image width", false);
        widthParam.setMin("1");
        widthParam.setMax(to_string(width.GetMax()));
        ...
        auto heightParam = SDK::v1::CameraParameter::CreateParameter(
            SDK::v1::ParameterType::Int, "Height", to_string(height.GetValue()), "Image height", false);
        heightParam.setMin("1");
        heightParam.setMax(to_string(height.GetMax()));
    ...
    SDK::v1::CameraParameterStatuses baslerCamera::setConfig(const SDK::v1::CameraParameters& parametersToChange)
    {
        ...
        for (const auto& parameterToChange : parametersToChange)
        {
            ...
            {
                status =
                    CEnumParameter(m_camera->GetNodeMap(), "BslLightControlMode").TrySetValue(parameterValue.c_str());
            }
            else if (parameterName == "BslLightDeviceSelector")
            {
                status = CEnumParameter(m_camera->GetNodeMap(), "BslLightDeviceSelector")
                             .TrySetValue(parameterValue.c_str());
            }
            else if (parameterName == "BslLightDeviceCurrent")
            {
                status = CFloatParameter(m_camera->GetNodeMap(), "BslLightDeviceCurrent")
                             .TrySetValue(std::stof(parameterValue));
            }
            else if (parameterName == "BslLightDeviceBrightness")
            {
                status = CFloatParameter(m_camera->GetNodeMap(), "BslLightDeviceBrightness")
                             .TrySetValue(std::stof(parameterValue));
            }
            else if (parameterName == "BslLightDeviceOperationMode")
            {
                status = CEnumParameter(m_camera->GetNodeMap(), "BslLightDeviceOperationMode")
                             .TrySetValue(parameterValue.c_str());
            }
